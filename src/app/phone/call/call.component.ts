			...
			*/
			/*
						...
						if (this.oSipStack.start() != 0) {
							...
							this.txtRegStatus = '<b>Failed to start the SIP stack</b>';
						}
						else return;
			*/
			...
			let aor = "sip:test.user@zdslogic.com";
			aor = this.callForm.controls['txtPublicIdentity'].value;
			...
			authorizationUsername = this.callForm.controls['txtPrivateIdentity'].value;
			...
			authorizationPassword = this.callForm.controls['txtPassword'].value;;
...
*/
			...
			this.txtRegStatus = "<i>" + "Connected" + "</i>";
	...
	sipUnRegister = () => {
		...
		this.txtRegStatus = "<i>" + "Disconnected" + "</i>";
	...
	sipHangUp = () => {
		...
		*/
		...
		this.simpleUser.hangup().catch((error: Error) => {
			...
			console.error(error);
			alert("Failed to hangup call.\n" + error);
			this.txtCallStatus = '<i>Failed to hangup call...</i>';

		});

		this.txtCallStatus = '<i>Call Terminated</i>';
	...
	loadDefaults = () => {
		this.callForm.controls['txtDisplayName'].setValue("ZdsLogic Technologies");
		this.callForm.controls['txtPrivateIdentity'].setValue("test.user");
		this.callForm.controls['txtPublicIdentity'].setValue("sip:test.user@zdslogic.com");
		...
		this.callForm.controls['txtPhoneNumber'].setValue("sip:1111@zdslogic.com");
		this.callForm.controls['txtWebsocketServerUrl'].setValue("wss:/zdslogic.com:6089/ws");
		this.callForm.controls['txtSIPOutboundProxyUrl'].setValue("udp://zdslogic.com:5060");
